    ...
    return new Promise((resolve, reject) => {
        ...
        dbRequest.onupgradeneeded = (event) => {
            ...
            if (!db.objectStoreNames.contains('persons')) {
                ...
                console.log('[PersonMaster] Person object store created');
            }
        };

        dbRequest.onsuccess = () => {
            personDB = dbRequest.result;
            console.log('[PersonMaster] IndexedDB initialized successfully');
            resolve(personDB);
        };
    });
}
...
function getAllPersons() {
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
            console.log('[PersonMaster] Retrieved all persons:', request.result);
...
function addPerson(personData) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            request.onsuccess = () => {
                console.log('[PersonMaster] Person added successfully:', person);
                resolve(person);
            };
...
function updatePerson(id, personData) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            request.onsuccess = () => {
                console.log('[PersonMaster] Person updated successfully:', updatedPerson);
...
function deletePerson(id) {
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
            console.log('[PersonMaster] Person deleted successfully:', id);
            resolve(true);
        };
...
function exportAllPersons() {
    return getAllPersons().then(persons => {
        ...
        console.log('[PersonMaster] Persons exported successfully');
        return true;
    });
}
...
function clearAllPersons() {
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
            console.warn('[PersonMaster] All persons cleared');
            resolve(true);
        };
...
document.addEventListener('DOMContentLoaded', () => {
    initializePersonDatabase()
        .then(() => {
            console.log('[PersonMaster] Database ready');
            // Trigger any pending initialization
            if (window.onPersonDatabaseReady) {
                window.onPersonDatabaseReady();
            }
        })
        .catch(error => {
            console.error('[PersonMaster] Database initialization failed:', error);
            showErrorMessage('Database initialization failed: ' + error.message);
        });
});

console.log('[PersonMaster] Module loaded successfully');
